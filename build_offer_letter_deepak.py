#!/usr/bin/env python3
"""
Build a professional Offer Letter for Deepak Kumar, overlaid onto the
Crownest Hospitality LLP letterhead PDF (exact original design preserved).

Same letterhead and content layout as build_offer_letter.py, with the
candidate details filled in:
  - Name:        Deepak Kumar
  - Address:     Vill-Parthala Radha Kunj Colony Shiv Mandir,
                 Post-Gautam Buddh Nagar, Distt.-Gautam Buddh Nagar,
                 Uttar Pradesh - 201301
  - Mobile No.:  9958821146
  - Designation: House Keeper
  - Salary:      Rs. 16,500 /- per month
  - Duration:    (removed as requested)

Run:  python3 build_offer_letter_deepak.py
Out:  Offer_Letter_Deepak_Kumar.pdf
"""
...
OUTPUT = "Offer_Letter_Deepak_Kumar.pdf"
...
def build_story():
    ...
    s.append(Paragraph("<b>Deepak Kumar</b>", ADDR))
    s.append(Paragraph("Vill-Parthala Radha Kunj Colony Shiv Mandir,", ADDR))
    s.append(Paragraph("Post-Gautam Buddh Nagar,", ADDR))
    s.append(Paragraph("Distt.-Gautam Buddh Nagar,", ADDR))
    s.append(Paragraph("Uttar Pradesh - 201301", ADDR))
    s.append(Paragraph("Mobile No.: 9958821146", ADDR))
    s.append(Spacer(1, 6))

    s.append(Paragraph("Subject: Offer of Employment", SUBJECT))

    s.append(Paragraph("Dear Deepak,", BODY))
    s.append(Paragraph(
        "We are pleased to offer you the position of "
        "<b>House Keeper</b> at <b>Crownest Hospitality LLP</b> "
    ...
    s.append(ListFlowable(
        [
            ListItem(Paragraph("<b>Designation:</b> House Keeper", BULLET), leftIndent=6),
            ListItem(Paragraph(
                "<b>Compensation:</b> Your salary will be Rs. 16,500/- per month, "
    ...
    s.append(Paragraph(
        "<b>Accepted &amp; agreed</b> (Deepak Kumar) &nbsp;&mdash;&nbsp; "
